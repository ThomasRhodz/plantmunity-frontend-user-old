...
import { useUpdateShopMutation } from '../../../app/services/shopApi';
...
const EditShopForm = ({ handleClose, id, name, logo, email, contact, telephone, address, tOpen, tClose,bio, tags, toast}) => {
    ...
    const arrayTags = tags === null ? [] : tags.split(',');

    const [imageUpload, setImageUpload] = useState(logo === null ? '' : logo );
    const [upload, setUpload] = useState(logo === null ? false : true);
    const [shop_name, setShopName] = useState(name);
    const [shop_telephone, setTelephone] = useState(telephone === null ? '' :telephone );
    const [shop_email, setEmail] = useState(email);
    const [shop_address, setAddress] = useState(address);
    const [shop_contact, setContact] = useState(contact === null ? '' : contact);
    const [shop_bio, setBio] = useState(bio === null ? '' : bio);
    const [tag, setTag] = useState('');
    const [shop_tags, setTags] = useState(arrayTags);
